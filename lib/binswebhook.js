...
const DEFAULT_CSL463_SIGNAL_APPEARANCE = false;
...
class BinsWebhook {
  ...
  constructor(options) {
    ...
    this.csl463SignalAppearance = DEFAULT_CSL463_SIGNAL_APPEARANCE;

    if(options.hasOwnProperty('csl463SignalAppearance')) {
      self.csl463SignalAppearance = options.csl463SignalAppearance;
    }
...
function handleNewDevice(instance) {
  if(instance.csl463SignalAppearance) {
    clearTimeout(instance.signalAppearanceTimeoutId);
    exec("/opt/inf_out_set 0 1", (error, stdout, stderr) => {
...
function handleSignalAppearanceTimeout() {
  exec("/opt/inf_out_set 0 0", (error, stdout, stderr) => {
    if(error) { console.log(error); }
  });
}
